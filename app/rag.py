...
def answer_with_rag(question: str, k: int = 5) -> tuple[str, List[Document]]:
    ...
    chat = make_chat()
    # prompt is list[BaseMessage], convert to simple dicts for adapter
    msg_input = [{"role": m.type, "content": m.content} for m in prompt]
    output = chat.invoke(msg_input)
...
def _call_llm_from_prompt(messages: List[Dict[str, Any]]):
    """Adapter: list-of-message-dicts -> make_chat.invoke"""
    chat = make_chat()
...
def _render_and_call(inp: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Call LLM
    out = _call_llm_from_prompt(msg_input)
    text = getattr(out, "content", out)
    return {"answer": text, "docs": inp.get("docs", [])}
